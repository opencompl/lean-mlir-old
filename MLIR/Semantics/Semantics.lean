...
def OpM.bind [Code type] (ma: OpM type A) (a2mb: A -> OpM type B): OpM type B :=
  match ma with
  | .Unhandled s => .Unhandled s
  | .Ret a => a2mb a
  | .Error s => .Error s
...
instance [Code type]: Monad (OpM type) where
   pure := OpM.Ret
   bind := OpM.bind
...
abbrev TopM (type: Type) [ct: Code type] (R: Type _) := StateT (SSAEnv type) (Except (String × (SSAEnv type))) R
def TopM.run [ct: Code type] (t: TopM type R) (env: SSAEnv type): Except (String × (SSAEnv type)) (R × SSAEnv type) :=
  StateT.run t env

def TopM.scoped [ct: Code type] (t: TopM type R): TopM type R := do
...
def TopM.raiseUB [ct: Code type] (message: String): TopM type R := do
  let state ← get
  Except.error (message, state)

def TopM.get [ct: Code type] (τ: MLIRType type) (name: SSAVal): TopM type τ.eval := do
  let s ← StateT.get
  match SSAEnv.get name τ s with
  | .some v => return v
  | .none => return default

def TopM.set [ct: Code type] (τ: MLIRType type) (name: SSAVal) (v: τ.eval): TopM type Unit := do
  let s ← StateT.get
  match SSAEnv.get name τ s with
...
class Semantics (attr type: Type) [ct: Code type] [ca: Code attr]  where
  ...
  -- program's entire dialect type but returns none for any operation that is not
  -- part of δ.
...
def denoteTypedArgs [ct: Code type] (args: TypedArgs type) (names: List SSAVal): TopM type Unit :=
 match args with
 | [] => return ()
 | ⟨τ, val⟩::args =>
    match names with
...
def TopM.denoteRegionsByIx [ct: Code type]
  (rs0: List (TypedArgs type → TopM type (TypedArgs type)))
 (ix: Nat) (args: TypedArgs type): TopM type (TypedArgs type) :=
  match rs0 with
...
instance
    ...
    [S₂: Semantics δ₂]
    : Semantics (δ₁ + δ₂) where
  -- semantics_op: IOp type → Fitree (RegionE type +' UBE) (BlockResult type)
  semantics_op op :=
