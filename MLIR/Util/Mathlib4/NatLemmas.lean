...
import MLIR.Util.Mathlib4.AlgebraFunctions
import MLIR.Util.Mathlib4.Logic
...
protected theorem pos_of_ne_zero {n : ℕ} : n ≠ 0 → 0 < n :=
...
-- Port note: this theorem was not present in Lean 3
protected theorem add_left_cancel_iff {n m k : ℕ} : n + m = n + k ↔ m = k :=
...
protected theorem lt_add_right (a b c : Nat) : a < b → a < b + c :=
...
| n+1, h =>
  ...
  Nat.noConfusion h1 (fun h => by simp at h)
...
protected theorem sub_lt_of_pos_le (a b : ℕ) (h₀ : 0 < a) (h₁ : a ≤ b)
: b - a < b :=
...
@[simp] protected theorem min_eq_min (a : ℕ) : Nat.min a b = min a b := rfl
...
protected theorem case_strong_induction_on {p : Nat → Prop} (a : Nat)
  (hz : p 0)
...
protected theorem sub_le_sub_left (k : ℕ) (h : n ≤ m) : k - m ≤ k - n :=
...
theorem pred_inj : ∀ {a b : ℕ}, 0 < a → 0 < b → Nat.pred a = Nat.pred b → a = b
...
protected theorem find_spec : p (Nat.find p H) := (Nat.find_x p H).2.1

protected theorem find_min : ∀ {m : ℕ}, m < Nat.find p H → ¬p m := @(Nat.find_x p H).2.2

protected theorem find_min' {m : ℕ} (h : p m) : Nat.find p H ≤ m :=
...
@[simp] theorem add_mul_mod_self_left (x y z : ℕ) : (x + y * z) % y = x % y := by
  induction z with
...
protected theorem div_self (H : 0 < n) : n / n = 1 :=
...
protected theorem div_eq_of_lt_le
...
theorem to_digits_core_lens_eq_aux (b f : Nat) :
...
theorem to_digits_core_lens_eq (b f : Nat) : ∀ (n : Nat) (c : Char) (tl : List Char),
...
theorem to_digits_core_length (b : Nat) (h : 2 <= b) (f n e : Nat)
