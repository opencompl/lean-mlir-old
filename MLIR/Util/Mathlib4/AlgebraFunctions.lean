...
import MLIR.Util.Mathlib4.AlgebraOrder
...
theorem min_left_comm : @left_commutative α α min :=
left_comm min (@min_comm α _) (@min_assoc α _)

@[simp]
theorem min_self (a : α) : min a a = a := by simp[min]
...
@[simp]
theorem max_self (a : α) : max a a = a := by simp[max]
