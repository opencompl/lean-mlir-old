...
-- Workaround for not being able to add ext theorems from other modules.
private def funext' := @funext
private def propext' := @propext

protected theorem Unit.ext (x y : Unit) : x = y := rfl
protected theorem PUnit.ext (x y : Unit) : x = y := rfl
...
theorem proof_irrel : type_of% @proofIrrel := proofIrrel
theorem congr_fun : type_of% @congrFun := congrFun
theorem congr_arg : type_of% @congrArg := congrArg
...
theorem eq_rec_heq : type_of% @eqRec_heq := @eqRec_heq
...
theorem Iff.elim (f : (a → b) → (b → a) → c) (h : a ↔ b) : c := f h.1 h.2
...
theorem iff_true_intro (h : a) : a ↔ True := ⟨fun _ => ⟨⟩, fun _ => h⟩

theorem iff_false_intro (h : ¬a) : a ↔ False := ⟨h, fun h => h.elim⟩

theorem not_iff_false_intro (h : a) : ¬a ↔ False := iff_false_intro (not_not_intro h)

theorem not_non_contradictory_iff_absurd (a : Prop) : ¬¬¬a ↔ ¬a := ⟨mt not_not_intro, not_not_intro⟩
...
theorem and_implies (hac : a → c) (hbd : b → d) : a ∧ b → c ∧ d := And.imp hac hbd
...
theorem and_assoc (a b : Prop) : (a ∧ b) ∧ c ↔ a ∧ (b ∧ c) := And.assoc

theorem And.left_comm : a ∧ (b ∧ c) ↔ b ∧ (a ∧ c) :=
by rw [← and_assoc, ← and_assoc, @And.comm a b]; constructor <;> exact id
...
namespace Decidable
  ...
  theorem by_contradiction [φ : Decidable p] (h : ¬ p → False) : p := @byContradiction p φ h
...
theorem Bool.ff_ne_tt : false = true → False := Bool.noConfusion
...
λ (x y : α) =>
 ...
 else isFalse (λ hxy : x = y => absurd (h₂ y) (by rw [hxy] at hp; assumption))
...
theorem decidable_eq_inr_neg {α : Sort u} [h : DecidableEq α] {a b : α} : ∀ n : a ≠ b, h a b = isFalse n :=
λ n =>
match (h a b) with
| isTrue e   => absurd e n
...
theorem if_congr_prop {b c x y u v : Prop} [Decidable b] [Decidable c]
                    (h_c : b ↔ c) (h_t : x ↔ u) (h_e : y ↔ v) :
        ite b x y ↔ ite c u v :=
