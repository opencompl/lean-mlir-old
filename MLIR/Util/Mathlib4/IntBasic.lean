...
import MLIR.Util.Mathlib4.TacticBasic
...
theorem ofNat_zero : ofNat (0 : ℕ) = (0 : ℤ) := rfl

theorem ofNat_one  : ofNat (1 : ℕ) = (1 : ℤ) := rfl
...
protected theorem neg_zero : -(0:ℤ) = 0 := rfl

theorem ofNat_add (n m : ℕ) : ofNat (n + m) = ofNat n + ofNat m := rfl
theorem ofNat_mul (n m : ℕ) : ofNat (n * m) = ofNat n * ofNat m := rfl
theorem ofNat_succ (n : ℕ) : ofNat (succ n) = ofNat n + 1 := rfl

theorem neg_ofNat_zero : -(ofNat 0) = 0 := rfl
theorem neg_ofNat_of_succ (n : ℕ) : -(ofNat (succ n)) = -[1+ n] := rfl
theorem neg_neg_ofNat_succ (n : ℕ) : -(-[1+ n]) = ofNat (succ n) := rfl

theorem negSucc_ofNat_coe (n : ℕ) : -[1+ n] = -↑(n + 1) := rfl

/- ## These are only for internal use -/

theorem ofNat_add_ofNat (m n : ℕ) :
    ofNat m + ofNat n = ofNat (m + n) := rfl
theorem ofNat_add_negSucc_ofNat (m n : ℕ) :
    ofNat m + -[1+ n] = subNatNat m (succ n) := rfl
theorem negSucc_ofNat_add_ofNat (m n : ℕ) :
    -[1+ m] + ofNat n = subNatNat n (succ m) := rfl
theorem negSucc_ofNat_add_negSucc_ofNat (m n : ℕ) :
    -[1+ m] + -[1+ n] = -[1+ succ (m + n)] := rfl

theorem ofNat_mul_ofNat (m n : ℕ) :
    ofNat m * ofNat n = ofNat (m * n) := rfl
theorem ofNat_mul_negSucc_ofNat (m n : ℕ) :
    ofNat m * -[1+ n] = negOfNat (m * succ n) := rfl
theorem negSucc_ofNat_ofNat (m n : ℕ) :
    -[1+ m] * ofNat n = negOfNat (succ m * n) := rfl
theorem mul_negSucc_ofNat_negSucc_ofNat (m n : ℕ) :
...
theorem ofNat_eq_ofNat_iff (m n : ℕ) : ofNat m = ofNat n ↔ m = n :=
...
protected theorem sub_eq_add_neg {a b : ℤ} : a - b = a + -b := rfl

/- ## basic properties of subNatNat -/

theorem subNatNat_elim (m n : ℕ) (P : ℕ → ℕ → ℤ → Prop)
...
theorem subNatNat_add_left {m n : ℕ} :
...
theorem subNatNat_add_right {m n : ℕ} : subNatNat m (m + n + 1) = negSucc n := by
...
theorem natAbs_ofNat (n : ℕ) : natAbs ↑n = n := rfl

theorem eq_zero_ofNatAbs_eq_zero : ∀ {a : ℤ}, natAbs a = 0 → a = 0
...
theorem natAbs_zero : natAbs (0 : ℤ) = (0 : ℕ) := rfl

theorem natAbs_one : natAbs (1 : ℤ) = (1 : ℕ) := rfl

theorem natAbs_mul_self : ∀ {a : ℤ}, ↑(natAbs a * natAbs a) = a * a
| (ofNat _) => rfl
| -[1+ _]   => rfl

@[simp] theorem natAbs_neg (a : ℤ) : natAbs (-a) = natAbs a := match a with
| (ofNat zero)     => rfl
| (ofNat (succ _)) => rfl
| (negSucc _)      => rfl
...
theorem eq_x_or_neg (a : ℤ) : ∃n : ℕ, a = n ∨ a = -↑n := ⟨_, natAbs_eq a⟩
...
theorem subNatNat_sub {m n : ℕ} (h : n ≤ m) (k : ℕ) :
...
theorem subNatNat_add_negSucc_ofNat (m n k : ℕ) :
...
theorem negSucc_ofNat_mul_negOfNat (m : ℕ) :
  ∀ n, -[1+ m] * negOfNat n = ofNat (succ m * n)
| 0      => rfl
| succ _ => rfl

theorem negOfNat_mul_negSucc_ofNat (m n : ℕ) :
...
theorem ofNat_mul_subNatNat (m n k : ℕ) :
...
theorem negSucc_ofNat_mul_subNatNat (m n k : ℕ) :
...
protected theorem zero_ne_one : (0 : ℤ) ≠ 1 := λ h => Int.noConfusion h (fun h => by simp at h)

theorem ofNat_sub {n m : ℕ} (h : m ≤ n) : ofNat (n - m) = ofNat n - ofNat m := by
...
protected theorem neg_add {a b : ℤ} : - (a + b) = -a + -b := by
...
theorem negSucc_ofNat_coe' (n : ℕ) : -[1+ n] = -↑n - 1 :=
by rw [Int.sub_eq_add_neg, ← Int.neg_add]; rfl

protected theorem subNatNat_eq_coe {m n : ℕ} : subNatNat m n = ↑m - ↑n := by
