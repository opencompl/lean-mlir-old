  ...
  | .some data => do
        let out <- Fitree.trigger (RegionE.RunRegion (Δ := Δ) (ix := 0)
                -- TODO, @lephe: please check that my theorem is correct!
                   (args := [⟨ τ, MLIRTy_eval_equal_after_coe (Δ := Δ) τ ▸ data ⟩]))
        match out with
        | .Ret [⟨ σ, v ⟩] =>
               return (if H: σ = τ
                 then  .some (MLIRTy_eval_equal_after_coe (Δ := Δ) τ ▸ (H ▸ v))
                 else .none)
        | _ => return .none
  | .none => do
      Fitree.trigger (UBE.UB "unable to access tensor data")
      return .none
...
def linalg_semantics_op  [CoeDialect builtin Δ] [P: DialectProjection Δ builtin]: IOp Δ →
  ...
  | IOp.mk "linalg.parallel2d1" (resTy := []) (args := [⟨.extended sΔ, v⟩]) [] 1 attrs =>
...
theorem INPUT (input: RankedTensor [.Known 2, .Known 4] .i32): SSAEnv builtin :=
      SSAEnv.One [("input", ⟨builtin.tensor [.Known 2, .Known 4] .i32, input⟩)]
