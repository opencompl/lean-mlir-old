...
"builtin.module"() ({
  "gpu.module"() ({
    "gpu.func"() ({
    ^bb0(%arg0: memref<8 × i32>, %arg1: memref<8 × 8 × i32>, %arg2: memref<8 × 8 × 8 × i32>):
      ...
      %4 = "memref.load"(%arg1, %1, %0) : (memref<8 × 8 × i32>, index, index) -> i32
      %5 = "arith.addi"(%3, %4) : (i32, i32) -> i32
      "memref.store"(%5, %arg2, %2, %1, %0) : (i32, memref<8 × 8 × 8 × i32>, index, index, index) -> ()
      "gpu.return"() : () -> ()
    }) {function_type = (memref<8 × i32>, memref<8 × 8 × i32>, memref<8 × 8 × 8 × i32>) -> (), gpu.kernel, spv.entry_point_abi = {local_size = dense<1> : vector<3 × i32>}, sym_name = "kernel_addi", workgroup_attributions = 0 : i64} : () -> ()
    "gpu.module_end"() : () -> ()
  }) {sym_name = "kernels"} : () -> ()
  "func.func"() ({
    %0 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<8 × i32>
    %1 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<8 × 8 × i32>
    ...
    "gpu.launch_func"(%10, %10, %10, %9, %9, %9, %0, %1, %2) {kernel = @kernels::@kernel_addi, operand_segment_sizes = dense<[0, 1, 1, 1, 1, 1, 1, 0, 3]> : vector<9 × i32>} : (index, index, index, index, index, index, memref<8 × i32>, memref<8 × 8 × i32>, memref<8 × 8 × 8 × i32>) -> ()
  ...
  "func.func"() ({
  }) {function_type = (memref<? × ? × i32>, i32) -> (), sym_name = "fillResource2DInt", sym_visibility = "private"} : () -> ()
  "func.func"() ({
...
def main : IO Unit :=
    ...
    FS.writeFile "testZmlir-vulkan-runnerZaddi.out.txt" str
