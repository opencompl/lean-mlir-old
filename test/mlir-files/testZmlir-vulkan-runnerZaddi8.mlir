"builtin.module"() ({
  "gpu.module"() ({
    "gpu.func"() ({
    ^bb0(%arg0: memref<8 × i8>, %arg1: memref<8x8xi8>, %arg2: memref<8x8x8xi32>):
      ...
      "gpu.return"() : () -> ()
    }) {function_type = (memref<8 × i8>, memref<8x8xi8>, memref<8x8x8xi32>) -> (), gpu.kernel, spv.entry_point_abi = {local_size = dense<1> : vector<3 × i32>}, sym_name = "kernel_addi", workgroup_attributions = 0 : i64} : () -> ()
    "gpu.module_end"() : () -> ()
  }) {sym_name = "kernels"} : () -> ()
  "func.func"() ({
    %0 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<8 × i8>
    %1 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<8x8xi8>
    ...
    "gpu.launch_func"(%10, %10, %10, %9, %9, %9, %0, %1, %2) {kernel = @kernels::@kernel_addi, operand_segment_sizes = dense<[0, 1, 1, 1, 1, 1, 1, 0, 3]> : vector<9 × i32>} : (index, index, index, index, index, index, memref<8 × i8>, memref<8x8xi8>, memref<8x8x8xi32>) -> ()
