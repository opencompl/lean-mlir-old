...
"builtin.module"() ({
  ...
  "func.func"() ({
    ...
    %2 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<2x3xf32>
    ...
    %5 = "memref.cast"(%2) : (memref<2x3xf32>) -> memref<* × f32>
  ...
  ^bb0(%arg0: memref<2x3xf32>):
    %0 = "memref.reinterpret_cast"(%arg0) {operand_segment_sizes = dense<[1, 0, 0, 0]> : vector<4 × i32>, static_offsets = [0], static_sizes = [6, 1], static_strides = [1, 1]} : (memref<2x3xf32>) -> memref<6x1xf32>
    %1 = "memref.cast"(%0) : (memref<6x1xf32>) -> memref<* × f32>
  ...
  ^bb0(%arg0: memref<2x3xf32>):
    ...
    %3 = "memref.reinterpret_cast"(%arg0, %0, %1, %2, %2, %1) {operand_segment_sizes = dense<[1, 1, 2, 2]> : vector<4 × i32>, static_offsets = [-9223372036854775808], static_sizes = [-1, -1], static_strides = [-9223372036854775808, -9223372036854775808]} : (memref<2x3xf32>, index, index, index, index, index) -> memref<?x?xf32, #map>
    %4 = "memref.cast"(%3) : (memref<?x?xf32, #map>) -> memref<* × f32>
    "func.call"(%4) {callee = @print_memref_f32} : (memref<* × f32>) -> ()
    "func.return"() : () -> ()
  }) {function_type = (memref<2x3xf32>) -> (), sym_name = "cast_ranked_memref_to_dynamic_shape"} : () -> ()
  "func.func"() ({
  ^bb0(%arg0: memref<2x3xf32>):
    %0 = "memref.cast"(%arg0) : (memref<2x3xf32>) -> memref<* × f32>
    %1 = "memref.reinterpret_cast"(%0) {operand_segment_sizes = dense<[1, 0, 0, 0]> : vector<4 × i32>, static_offsets = [0], static_sizes = [6, 1], static_strides = [1, 1]} : (memref<* × f32>) -> memref<6x1xf32>
    %2 = "memref.cast"(%1) : (memref<6x1xf32>) -> memref<* × f32>
  ...
  ^bb0(%arg0: memref<2x3xf32>):
    ...
    %4 = "memref.reinterpret_cast"(%0, %1, %2, %3, %3, %2) {operand_segment_sizes = dense<[1, 1, 2, 2]> : vector<4 × i32>, static_offsets = [-9223372036854775808], static_sizes = [-1, -1], static_strides = [-9223372036854775808, -9223372036854775808]} : (memref<* × f32>, index, index, index, index, index) -> memref<?x?xf32, #map>
    %5 = "memref.cast"(%4) : (memref<?x?xf32, #map>) -> memref<* × f32>
    "func.call"(%5) {callee = @print_memref_f32} : (memref<* × f32>) -> ()
    "func.return"() : () -> ()
