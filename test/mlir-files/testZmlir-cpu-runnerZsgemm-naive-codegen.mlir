...
"builtin.module"() ({
  "func.func"() ({
    ...
    }) {operand_segment_sizes = dense<1> : vector<2 × i32>} : (f32, memref<16 × 16 × f32>) -> ()
    "linalg.fill"(%3, %1) ({
    ^bb0(%arg0: f32, %arg1: f32):
      "linalg.yield"(%arg0) : (f32) -> ()
    }) {operand_segment_sizes = dense<1> : vector<2 × i32>} : (f32, memref<16 × 16 × f32>) -> ()
    ...
    ^bb0(%arg0: index):
      ...
      "func.call"(%0, %1, %2) {callee = @sgemm_naive} : (memref<16 × 16 × f32>, memref<16 × 16 × f32>, memref<16 × 16 × f32>) -> ()
      "affine.yield"() : () -> ()
    }) {lower_bound = #map0, step = 1 : index, upper_bound = #map1} : () -> ()
    %6 = "func.call"() {callee = @rtclock} : () -> f64
    %7 = "arith.subf"(%6, %5) : (f64, f64) -> f64
    %8 = "affine.load"(%2) {map = #map2} : (memref<16 × 16 × f32>) -> f32
    "vector.print"(%8) : (f32) -> ()
    %9 = "arith.constant"() {value = 0 : index} : () -> index
    %10 = "arith.constant"() {value = 1 : index} : () -> index
    %11 = "arith.constant"() {value = 2 : index} : () -> index
    %12 = "memref.dim"(%2, %9) : (memref<16 × 16 × f32>, index) -> index
    %13 = "memref.dim"(%2, %10) : (memref<16 × 16 × f32>, index) -> index
    %14 = "memref.dim"(%0, %10) : (memref<16 × 16 × f32>, index) -> index
    ...
    "memref.dealloc"(%0) : (memref<16 × 16 × f32>) -> ()
    "memref.dealloc"(%1) : (memref<16 × 16 × f32>) -> ()
    "memref.dealloc"(%2) : (memref<16 × 16 × f32>) -> ()
    "func.return"() : () -> ()
  }) {function_type = () -> (), sym_name = "main"} : () -> ()
  "func.func"() ({
  ^bb0(%arg0: memref<16 × 16 × f32>, %arg1: memref<16 × 16 × f32>, %arg2: memref<16 × 16 × f32>):
    ...
    ^bb0(%arg3: index):
      ...
      ^bb0(%arg4: index):
        ...
        %2 = "affine.load"(%arg2, %arg3, %arg4) {map = #map3} : (memref<16 × 16 × f32>, index, index) -> f32
        "affine.store"(%2, %1, %0) {map = #map4} : (f32, memref<1 × f32>, index) -> ()
        "affine.for"() ({
        ^bb0(%arg5: index):
          %4 = "affine.load"(%arg0, %arg3, %arg5) {map = #map3} : (memref<16 × 16 × f32>, index, index) -> f32
          %5 = "affine.load"(%arg1, %arg5, %arg4) {map = #map3} : (memref<16 × 16 × f32>, index, index) -> f32
        ...
        "affine.store"(%3, %arg2, %arg3, %arg4) {map = #map3} : (f32, memref<16 × 16 × f32>, index, index) -> ()
    ...
    "func.return"() : () -> ()
  }) {function_type = (memref<16 × 16 × f32>, memref<16 × 16 × f32>, memref<16 × 16 × f32>) -> (), sym_name = "sgemm_naive"} : () -> ()
  "func.func"() ({
