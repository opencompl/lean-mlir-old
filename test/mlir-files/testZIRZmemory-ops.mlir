...
"builtin.module"() ({
  "func.func"() ({
    %0 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<1024x64xf32, 1>
    %1 = "arith.constant"() {value = 0 : index} : () -> index
    %2 = "arith.constant"() {value = 1 : index} : () -> index
    %3 = "memref.alloc"(%1, %2) {operand_segment_sizes = dense<[2, 0]> : vector<2 × i32>} : (index, index) -> memref<?x?xf32, 1>
    %4 = "memref.alloc"(%1) {operand_segment_sizes = dense<[0, 1]> : vector<2 × i32>} : (index) -> memref<2x4xf32, #map, 1>
    %5 = "memref.alloc"(%2, %1) {operand_segment_sizes = dense<1> : vector<2 × i32>} : (index, index) -> memref<2x?xf32, #map, 1>
    %6 = "memref.alloc"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<2 × i32>
    "func.return"() : () -> ()
  }) {function_type = () -> (), sym_name = "alloc"} : () -> ()
  "func.func"() ({
    %0 = "memref.alloca"() {operand_segment_sizes = dense<0> : vector<2 × i32>} : () -> memref<1024x64xf32, 1>
    %1 = "arith.constant"() {value = 0 : index} : () -> index
    %2 = "arith.constant"() {value = 1 : index} : () -> index
    %3 = "memref.alloca"(%1, %2) {operand_segment_sizes = dense<[2, 0]> : vector<2 × i32>} : (index, index) -> memref<?x?xf32, 1>
    %4 = "memref.alloca"(%1) {operand_segment_sizes = dense<[0, 1]> : vector<2 × i32>} : (index) -> memref<2x4xf32, #map, 1>
    %5 = "memref.alloca"(%2, %1) {operand_segment_sizes = dense<1> : vector<2 × i32>} : (index, index) -> memref<2x?xf32, #map, 1>
